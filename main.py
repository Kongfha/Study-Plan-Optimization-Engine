...
def load_config(args: argparse.Namespace) -> Dict:
    config = {
        ...
        "adhoc_hours": {},
        "daily_max_fatigue": DEFAULT_DAILY_MAX_FATIGUE,
        "daily_max_hours": 8.0,
        "use_sa_refine": True,
        "sa_iterations": 1000,
    }
...
def plan_subject(
    ...
    fatigue_per_hour_cap = config["daily_max_fatigue"] / max(config.get("daily_max_hours", 8.0), 1.0)
    high_fatigue_modules = [m.module_id for m in subject.modules if m.fatigue_drain > fatigue_per_hour_cap]
    if high_fatigue_modules:
        base_notes.append(
            f"Modules with fatigue per hour above the daily cap per hour ({fatigue_per_hour_cap:.1f}): {', '.join(high_fatigue_modules)}."
        )
...
def plan_global(
    ...
    for subject in subjects:
        ...
        fatigue_per_hour_cap = config["daily_max_fatigue"] / max(config.get("daily_max_hours", 8.0), 1.0)
        high_fatigue_modules = [m.module_key or m.module_id for m in subject.modules if m.fatigue_drain > fatigue_per_hour_cap]
        if high_fatigue_modules:
            notes.append(
                f"{subject.subject_name}: Modules above fatigue cap per hour ({fatigue_per_hour_cap:.1f}): {', '.join(high_fatigue_modules)}."
            )
...
def write_summary(
    output_dir: Path,
    start_date: date,
    subject_metrics: Dict[str, PlanMetrics],
    global_metrics: PlanMetrics | None = None,
) -> None:
    summary_payload = {
    ...
    for subject_name, metrics in subject_metrics.items():
        summary_payload["subjects"].append(
            {
                "subject_name": subject_name,
                "long_term": metrics.__dict__,
            }
        )

    if global_metrics:
        summary_payload["global"] = {
            "long_term": global_metrics.__dict__,
        }
